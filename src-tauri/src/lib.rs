...
//
// Two self-contained signals, both read straight from the image:
//  1. Sync mark (00 FF×10 00): a successfully-dumped raw sector always starts with
//     it; sectors that were never read (partial/interrupted dump) lack it.
//  2. EDC: every MODE1 / MODE2-XA sector embeds a CRC over its own contents. A
//     sector that was read with errors — or regenerated by a dumper to pad a gap —
//     keeps a plausible sync/header but fails its EDC. This is how IsoBuster
//     flags "read but damaged" sectors without any sidecar files.
// We report the contiguous LBA ranges failing either check so the UI can flag any
...
// CRC table for the CD-ROM EDC polynomial (ECMA-130 14.3), reflected form.
static EDC_LUT: OnceLock<[u32; 256]> = OnceLock::new();

fn edc_lut() -> &'static [u32; 256] {
    EDC_LUT.get_or_init(|| {
...
fn edc_compute(data: &[u8]) -> u32 {
    ...
    let mut edc = 0u32;
    for &b in data {
        edc = (edc >> 8) ^ lut[((edc ^ b as u32) & 0xFF) as usize];
    }
    edc
}
...
fn sector_edc_ok(sec: &[u8]) -> bool {
    ...
    match sec[15] & 0x0F {
        ...
        0 => true,
        // MODE1: EDC over sync..data (0..2064), stored at 2064.
        1 => edc_compute(&sec[..2064]) == stored(2064),
        // MODE2 XA: form bit in the subheader's submode byte.
        2 => {
            // The XA subheader is stored twice; plain MODE2 (no subheader, no
            // EDC) won't have matching copies — nothing to validate there.
            // (A corrupt XA sector can slip through this, but guessing a form
            ...
            if sec[18] & 0x20 != 0 {
                ...
                let st = stored(2348);
                st == 0 || edc_compute(&sec[16..2348]) == st
            } else {
                // Form 1: EDC over subheader..data (16..2072) at 2072.
                edc_compute(&sec[16..2072]) == stored(2072)
            }
        }
        // Unknown mode byte on a synced sector — corrupt header.
        _ => false,
    }
}
...
fn sector_bad(sec: &[u8], descramble: bool, scratch: &mut [u8; RAW_SECTOR_SIZE as usize]) -> bool {
    ...
    if descramble {
        let table = cdi_filesystem::scramble_table();
        scratch[..12].copy_from_slice(&sec[..12]);
...
fn scan_sync_gaps(file_path: &Path, base: u64, total: u64, descramble: bool) -> Vec<(u32, u32)> {
    ...
    // Sampling pre-check: if a spread of sectors all pass, treat as clean.
...
impl_extract_fs!(fat_filesystem::FatFs<F>, F: Read + Seek);
...
fn list_disc_contents(image_path: String, dir_path: String, filesystem: Option<String>, show_resource_forks: bool) -> Result<Vec<DiscEntry>, String> {
    ...
    // KryoFlux flux dump: decode the stream set to a sector image, mount FAT.
    if lower.ends_with(".raw") && kryoflux::is_kryoflux_stream(Path::new(path)) {
        let img = kryoflux::floppy_image(Path::new(path))?;
        return fat_filesystem::FatFs::new(std::io::Cursor::new(img))?.list_directory(&dir_path);
    }
...
fn extract_single_file(image_path: String, file_path: String, dest_path: String, filesystem: Option<String>) -> Result<(), String> {
    ...
    if lower.ends_with(".raw") && kryoflux::is_kryoflux_stream(Path::new(path)) {
        let img = kryoflux::floppy_image(Path::new(path))?;
        return fat_filesystem::FatFs::new(std::io::Cursor::new(img))?.extract_file(&file_path, &dest_path);
    }
...
async fn save_directory(cancel_state: tauri::State<'_, ExtractCancelState>, image_path: String, dir_path: String, dest_path: String, filesystem: Option<String>, apple_double: bool) -> Result<(), String> {
    ...
    if lower.ends_with(".raw") && kryoflux::is_kryoflux_stream(Path::new(path)) {
        let img = kryoflux::floppy_image(Path::new(path))?;
        return extract_tree!(cancel, fat_filesystem::FatFs::new(std::io::Cursor::new(img))?, &dir_path, &dest_path);
    }
